# Hi, I'm Mohit 👋

🚀 Frontend Team Lead with 6+ years of experience  
⚛️ React.js | Next.js | TypeScript | Tailwind | Angular  
👥 Led a team

## 🔥 What I Do
- Build scalable frontend architectures
- Optimize performance & SEO (Next.js)
- Code reviews & mentoring
- Convert Figma → pixel-perfect UI

## 🛠 Tech Stack
- React, Next.js, Angular
- JavaScript, TypeScript
- Tailwind, CSS, HTML
- Git, GitHub, CI/CD

## 📌 Featured Projects
- 🔗 File Sharing App (React / Next.js)
- 🔗 QR Code Generator
- 🔗 Vector Flow App
- 🔗 Live Chat
- 🔗 Dashboard Insights/Analytics
- 🔗 Omni-Channel Communication
- 🔗 Full Calendar
- 🔗 Socket IO
- 🔗 Automated Reputation Management (Review Response Automation)
  

## 📫 Contact
- Personal Website: https://mk-codely.vercel.app/
- LinkedIn: https://www.linkedin.com/in/mohit-kumar-umb
